...
def run_function_with_const(
    ...
    try:
        ...
        tol = g_data.get('tol', 1e-6)
        maxiter = g_data.get('maxiter', 1000)
        attempts = g_data.get('attempts', 50)
    ...
    if jac is None:
         logging.error(f"Jacobian for objective function {func_name} index {loc} not available for run {run_count}. SLSQP cannot proceed.")
         return None
...
def optimize_func(loc, funcs_minors, funcs_jacobians, config, eqs=[], count=0):
    ...
    try:
        ...
        logging.error(f"Config missing key for optimize_func (n, tol): {e}")
        return None
    ...
    return run_function_with_const(
        loc, constraints_list, funcs_minors, funcs_jacobians, config, func_name, count
    )
...
def optimize_constrained_dimension(
    ...
    try:
         g_data = config['global_data']
         n = g_data['n']
         num_workers = g_data.get('num_workers', None) # num_workers is optional
    except KeyError as e:
    ...
    logging.info(f"Running {num_tasks} tasks ({num_tasks // 2} points) using {num_workers or 'default'} workers.")
...
def run_optimization(config):
    ...
    try:
        g_data = config['global_data']
        funcs_to_optimize_config = g_data['funcs_to_optimize'] # e.g., [2, 3, 4]
        points_dim = g_data['points_dim']                     # e.g., [10, 10, 10]
        n = g_data['n']

        # --- NEW LOGIC: Prepend implicit S1 ---
        # The full sequence of functions involved (1-based for user understanding)
        funcs_sequence = [1] + funcs_to_optimize_config       # e.g., [1, 2, 3, 4]
        # Corresponding 0-based indices for code use
        s_indices = [f - 1 for f in funcs_sequence]           # e.g., [0, 1, 2, 3]
